...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    socket.on("orderCreated", (order: any) => {
      ...
      fetchAnalytics();
      toast.success(`Ðon hàng m?i t? ${order.customerName}!`, {
        description: `T?ng giá tr?: ${Number(order.totalPrice).toLocaleString("vi-VN")} VND`,
      ...
      <header className="mb-10 flex justify-between items-start">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            V? d?p t? nh?ng dóa hoa, hi?u qu? t? nh?ng con s?.
          </p>
        </div>
              ...
              {stats.chartData && stats.chartData.length > 0 ? (
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-300">
                  ...
                  <p className="text-sm font-medium text-slate-400 text-center px-8">
                    Khi có don hàng hoàn t?t, bi?u d? s? t? d?ng c?p nh?t.
                  </p>
                </div>
              )}
            </div>
              ...
              {liveOrders.length > 0 ? (
                liveOrders.map((order, idx) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-slate-400 line-clamp-2 italic font-medium leading-relaxed">
                      "{order.customMessage || "Ðon hàng m?i"}"
                    </p>
                ...
                <div className="py-20 text-center flex flex-col items-center">
                  ...
                  <p className="text-xs text-slate-600 font-medium italic text-center">
                    Chua có don hàng m?i trong th?i gian g?n dây.
                  </p>
                </div>
              )}
            </AnimatePresence>
